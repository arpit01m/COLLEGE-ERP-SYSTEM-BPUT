...
/**
 * Fetch all students from MySQL backend.
...
export async function fetchStudents() {
  try {
    ...
    if (json.data && Array.isArray(json.data) && json.data.length > 0) {
      return { success: true, data: json.data, source: json.source || 'database' };
    }
...
/**
 * Create/register a new student record in MySQL
 */
...
/**
 * Update an existing student record in MySQL
 */
...
/**
 * Delete a student record from MySQL
 */
...
export async function checkBackendHealth() {
  try {
    ...
    if (!res.ok) return { status: 'offline', mysql: 'unknown' };
    return await res.json();
  } catch {
